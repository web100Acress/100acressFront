...
function LoginForm({ inModal = false, onSwitchToRegister, preventRedirect = false }) {
  ...
  return (
    <>
      <div className={`relative p-2 md:p-3 ${inModal ? "w-full" : "max-sm:w-[95vw]"}`}>
        {/* Heading with Close Button */}
        <div className="mb-1.5">
          <div className="flex items-center justify-between">
            <h2 className="text-[20px] md:text-[24px] font-extrabold text-[#e53935] font-sans tracking-tight">Login to your account</h2>
            {inModal && (
              <button
                type="button"
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <div className="relative">
              ...
              />
              {passwordHide ? (
                <EyeOff size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 cursor-pointer" onClick={() => setPasswordHide(false)} />
              ) : (
                <Eye size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-700 cursor-pointer" onClick={() => setPasswordHide(true)} />
              )}
            </div>
          </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-[#e53935] hover:bg-[#c62828] disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-bold rounded-lg py-2 mt-0.5 text-sm shadow-sm hover:shadow-md transition"
          >
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2 border border-gray-300 rounded-lg py-2 hover:bg-gray-50 disabled:bg-gray-100 disabled:cursor-not-allowed transition"
          >
