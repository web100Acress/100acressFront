...
const LocationSection = ({ 
  ...
  return (
    <section className="relative py-8 md:py-10 lg:py-12 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white overflow-hidden">
               ...
               <div className="grid grid-cols-2 gap-4 mt-6 mb-2">
                  {tabsWithPoints.map((tab, index) => (
                    <motion.div
                      ...
                      className={`bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl p-3  border-gray-700/50 hover:border-${tab.color.split(' ')[2]}/30 transition-all duration-300 group cursor-pointer`}
                    >
                      ...
                      <div className="space-y-1">
                        {tab.points.slice(0, 2).map((point, i) => (
                          <div key={i} className="flex items-center space-x-3 group/item">
                            <div className={`w-8 h-8 mb-3 bg-gradient-to-br ${tab.color} rounded-lg flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}>
                              {tab.icon}
                            </div>
                            <p className="text-gray-300 text-xl leading-tight group-hover/item:text-white transition-colors duration-300 flex-1 font-semibold">{point}</p>
                          </div>
                        ))}
                        {tab.points.length > 2 && (
