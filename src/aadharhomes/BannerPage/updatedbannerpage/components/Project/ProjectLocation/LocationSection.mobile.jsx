    ...
    <section className="location-section-mobile">
      ...
      <div className="location-section-mobile-background">
        <div className="location-section-mobile-background-overlay">
          <div className="location-section-mobile-float-element-1"></div>
          <div className="location-section-mobile-float-element-2"></div>
        </div>
        <div className="location-section-mobile-gradient-overlay"></div>
      </div>

      <div className="location-section-mobile-content">
        {/* Section Header */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={isMounted ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6 }}
          className="location-section-mobile-header"
        >
          <motion.div 
            className="location-section-mobile-header-icon"
          ...
          <h2 className="location-section-mobile-subtitle">
            LOCATION & CONNECTIVITY
          </h2>
          <h3 className="location-section-mobile-title">
            Strategic Location with Unmatched Connectivity
          </h3>
          <div className="location-section-mobile-accent-line"></div>
        </motion.div>

        {/* Main Content */}
        <div className="location-section-mobile-main-grid">
          {/* Location Map Card */}
          <motion.div 
            className="location-section-mobile-map-card"
            initial={{ opacity: 0, x: -20 }}
            animate={isMounted ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.6, delay: 0.2 }}
          >
            <div className="location-section-mobile-map-glow"></div>
              
              {/* Map Container */}
              <div className="location-section-mobile-map-container">
                {/* Map Image */}
                <div className="location-section-mobile-map-image-container">
                  {locationImage?.url ? (
                    <img 
                      src={locationImage.url} 
                      alt={`${projectName} Location`} 
                      className="location-section-mobile-map-image"
                    />
                  ) : (
                    <div className="location-section-mobile-map-placeholder">
                      <div className="location-section-mobile-map-placeholder-content">
                        <div className="location-section-mobile-map-placeholder-icon">
                          <svg className="w-8 h-8 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-1.447-.894L15 4m0 13V4m0 0L9 7" />
                          </svg>
                        </div>
                        <p className="location-section-mobile-map-placeholder-text">Interactive Location Map</p>
                      </div>
                    </div>
                  )}
                  
                  {/* Map Overlay Controls */}
                  <div className="location-section-mobile-map-overlay">
                    <div className="location-section-mobile-map-overlay-content">
                      <button 
                        onClick={handleViewOnMap}
                        className="location-section-mobile-view-map-button"
                      >
                        View on Map
                      </button>
                    </div>
                ...
                {/* Location Info */}
                <div className="location-section-mobile-location-info">
                  <div className="location-section-mobile-location-info-content">
                    ...
                    <div className="location-section-mobile-location-details">
                      <h4 className="location-section-mobile-location-name">
                        {projectName || "Prime Location"}
                      </h4>
                      <p className="location-section-mobile-location-address">
                        {projectAddress && `${projectAddress}`}
                        {city && `, ${city}`}
                      </p>
                      <div className="location-section-mobile-location-badges">
                        <span className="location-section-mobile-badge amber">
                          Well Connected
                        </span>
                        <span className="location-section-mobile-badge blue">
                          Prime Location
                        </span>
                      </div>
                    </div>
          ...
          <motion.div 
            className="location-section-mobile-connectivity-grid"
            initial={{ opacity: 0, x: 20 }}
            animate={isMounted ? { opacity: 1, x: 0 } : {}}
          ...
          >
            {hasConnectivity && (
              <div className="h-full">
                <div className="location-section-mobile-connectivity-cards-grid">
                  {tabsWithPoints.map((tab, index) => (
                    <motion.div
                      ...
                      className="location-section-mobile-connectivity-card"
                    >
                      <div className="location-section-mobile-connectivity-card-header">
                        <div className={`location-section-mobile-connectivity-card-icon ${tab.color.split(' ')[2]}`}>
                          {tab.icon}
                        </div>
                      </div>
                      <div className="location-section-mobile-connectivity-points">
                        {tab.points.slice(0, 4).map((point, i) => (
                          <div key={i} className="location-section-mobile-connectivity-point">
                            <div className={`location-section-mobile-connectivity-point-dot ${tab.color.split(' ')[2]}`}></div>
                            <p className="location-section-mobile-connectivity-point-text">{point}</p>
                          </div>
                        ))}
                        {tab.points.length > 4 && (
                          <p className="location-section-mobile-more-points">+{tab.points.length - 4} more</p>
                        )}
                      </div>
                    </motion.div>
                  ))}
