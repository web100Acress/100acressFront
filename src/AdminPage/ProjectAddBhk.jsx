...
const ProjectsAddBhk = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://localhost:3500/bhk_view/${id}`);
  ...
  const submitBHKFromData = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`http://localhost:3500/bhk_insert/${id}`,editFromData);
  ...
  const handleDeleteUser = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:3500/bhk_delete/${id}`);
