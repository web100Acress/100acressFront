...
const CommonProject = ({ data, title, path ,animation, compact = false, showGrid = true, hideHeader = false }) => {
  ...
  return (
    <>
      {data?.length === 0 ? <CustomSkeleton /> : (
        <>
          <div data-aos={animation} className="py-0 w-full mx-0 font-['Rubik',sans-serif]">
            {!hideHeader && (
                   ...
                   return (
                     <span key={index} className="mb-2 md:mb-0">
                         ...
                         >
                           ...
                           {/* Body */}
                           <div className={`flex flex-col flex-1 justify-between ${compact ? 'p-3 gap-1.5' : 'p-4 gap-2'} font-['Rubik',sans-serif]`}>
                             <div>
                               {item?.projectName && (
                                <HeadingTag className={`${compact ? 'text-[15px]' : 'text-base md:text-[17px]'} font-semibold tracking-[-0.2px] text-gray-900 mb-1 group-hover:text-red-600 transition-colors truncate whitespace-nowrap font-['Rubik',sans-serif]`}>
                                  {item.projectName}
                                </HeadingTag>
                              )}
                             ...
                             {/* Price row */}
                             <div className="pt-0">
                               <span className={`inline-flex items-center gap-1 ${compact ? 'text-[14px]' : 'text-[15px]'} font-bold text-black whitespace-nowrap font-['Rubik',sans-serif]`}>
                                 <RupeeIcon />
