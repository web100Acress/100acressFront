  ...
  inFlightRequest = (async () => {
    try {
      ...
      if (response.data && response.data.data) {
        orderData = response.data.data.customOrders || response.data.data;
      } else if (response.data && typeof response.data === 'object') {
        orderData = response.data;
      }

      if (orderData && (orderData.luxury || orderData.farm || orderData.brandedresidences)) {
        // Update caches
        orderDataCache = orderData;
        lastCacheUpdate = Date.now();
        localStorage.setItem('projectOrders', JSON.stringify(orderData));
        console.log('✅ Project orders fetched and cached');
        return orderData;
      }
    } catch (error) {
...
// Moved hardcoded data to a constant for cleaner code
