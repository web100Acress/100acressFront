...
export const DataProvider = ({ children }) => {
  ...
  const fetchProject = async () => {
    try {
      const response = await axios.get('http://localhost:3500/project/viewAll/data');
  ...
  const fetchBlogData = async () => {
    try {
      const res = await axios.get("http://localhost:3500/blog/view");
  ...
  const handleUserLogin = async (userLogin) => {
    ...
    if (email && password) {
      try {
        const loginResponse = await axios.post(
          "http://localhost:3500/postPerson/verify_Login",
          { email, password }
        );
        ...
        if (loginResponse.status === 200) {
          const roleResponse = await axios.get(
            `http://localhost:3500/postPerson/Role/${email}`
          );
    ...
    const fetchCareerData = async () => {
      try {
        const res = await axios.get("http://localhost:3500/career/page/view");
  ...
  const fetchJobPostingData = async () => {
    try {
      const res = await axios.get(
        "http://localhost:3500/career/opening/ViewAll"
      );
  ...
  const buyFetchData = async () => {
    try {
      const res = await axios.get(
        "http://localhost:3500/property/buy/ViewAll"
      );
