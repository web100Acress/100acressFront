          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <div className="flex items-center gap-2">
              {items.slice(0,3).map(p => (
                <button key={p._id || p.id} className="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200 text-xs" onClick={() => onRemove?.(p)}>× {p.projectName?.slice(0,18)}</button>
              ))}
            </div>
          </div>
          <button onClick={onOpen} className="px-4 py-2 rounded-lg text-white" style={{background: gradients.primary}}>View Comparison</button>
        </div>
      </div>
    </div>
  );
