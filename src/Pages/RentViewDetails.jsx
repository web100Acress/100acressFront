...
`;
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await axios.get(
          `http://localhost:3500/property/view/${id}`
        );
        if (res.data.data) {
  ...
  const handleSubmitFormData = async (e) => {
    ...
    if (custNumber && custName) {
      try {
        const response = await axios.post("http://localhost:3500/postEnquiry", {
          ...userForm,
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get("http://localhost:3500/property/buy/ViewAll");
