const FormEnquiryReducer = (state, action) => {
  switch (action.type) {
    ...
    case "ALL_FRONT_ENQUIRES":
      ...
      const nonPendingEnquiries = allEnquiries.filter(
        (enquiry) => enquiry.status !== "Pending"
      );
    ...
    case "ALL_PROJECT_ENQUIRIES":
      ...
      const pendingProjectEnquiries = allProjectEnquiries.filter(
        (enquiry) => enquiry.status === "Pending"
      );
      const nonPendingProjectEnquiries = allProjectEnquiries.filter(
        (enquiry) => enquiry.status !== "Pending"
      );

      const sortedProjectEnquiries = [
        ...pendingProjectEnquiries,
        ...nonPendingProjectEnquiries,
      ];
      return {
        ...state,
