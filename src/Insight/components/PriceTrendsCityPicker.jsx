...
// API service for price trends banners
const getApiBaseUrl = () => {
  try {
    // Check if we're in a Node.js environment (SSR or build time)
...
// Lazy evaluation - get API URL when needed
const getApiUrl = (endpoint) => `${getApiBaseUrl()}${endpoint}`;

const fetchPriceTrendsBanners = async () => {
  try {
    const response = await fetch(getApiUrl('/api/insights-price-trends-banners'));
    if (!response.ok) throw new Error('Failed to fetch banners');
    return await response.json();
  } catch (error) {
    console.error('Error fetching price trends banners:', error);
    return { success: false, banners: [] };
  }
};

const fetchPriceTrendsSmallBanners = async () => {
  try {
    const response = await fetch(getApiUrl('/api/insights-price-trends-small-banners'));
    if (!response.ok) throw new Error('Failed to fetch small banners');
    return await response.json();
  } catch (error) {
    console.error('Error fetching price trends small banners:', error);
    return { success: false, banners: [] };
  }
};
...
export default function PriceTrendsCityPicker({
  ...
  // State for price trends banners
  ...
  useEffect(() => {
    const loadBanners = async () => {
      ...
      try {
        const [heroResponse, smallResponse] = await Promise.all([
          fetchPriceTrendsBanners(),
          fetchPriceTrendsSmallBanners()
        ]);

        if (heroResponse.success) {
          setHeroBanners(heroResponse.banners || []);
        } else {
          setBannersError('Failed to load hero banners');
        }

        if (smallResponse.success) {
          setSmallBanners(smallResponse.banners || []);
        } else {
          setBannersError('Failed to load small banners');
        }
      } catch (error) {
        ...
        {/* Background Image with Overlay */}
          <div className="absolute inset-0">
            {heroBanners.length > 0 ? (
              heroBanners.slice(0, 1).map((banner) => (
                <div key={banner._id}>
                  <img
                ...
                <div key={banner._id} className="mb-6">
                  ...
                  {banner.link && (
                    <a
                      ...
                      className="inline-flex items-center gap-2 px-6 py-3 bg-white/20 backdrop-blur-md border border-white/30 rounded-xl text-white font-semibold hover:bg-white/30 transition-all duration-200 shadow-lg"
                    >
                      Learn More
        ...
        {smallBanners.length > 0 && (
          <section className="mt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {smallBanners.slice(0, 6).map((banner) => (
                <div
                  key={banner._id}
                  className="group relative bg-white border-2 border-gray-200 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden"
                >
                    ...
                    <div className="absolute bottom-4 left-4 right-4">
                      ...
                      {banner.link && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 mt-3 px-4 py-2 bg-white/20 backdrop-blur-md border border-white/30 rounded-lg text-white text-sm font-medium hover:bg-white/30 transition-all duration-200"
                        >
                          Learn More
