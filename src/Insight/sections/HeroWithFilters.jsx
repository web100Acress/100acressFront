...
export default function HeroWithFilters() {
  ...
  const [buildersOpt, setBuildersOpt] = useState([{ value: "", label: "All Builders" }]);
  ...
  return (
    <section className="mb-4 md:mb-8" style={{ marginTop: 'calc(var(--nav-h, 64px) + 12px)' }}>
                    ...
                    <div className="fixed inset-0 z-30" aria-labelledby="advanced-filters-title" role="dialog" aria-modal="true">
                      ...
                      <div ref={dialogRef} className={`absolute left-1/2 top-1/2 -translate-x-1/2 w-[92vw] sm:w-[90vw] max-w-xl bg-gray-50 rounded-2xl shadow-2xl border border-gray-200 ${reducedMotion ? '-translate-y-1/2' : '-translate-y-[55%] transition-all duration-300'} p-4 md:p-6`} style={{ transform: 'translate(-50%, -50%)' }}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <select aria-label="Property Status" value={furnishing} onChange={(e)=>setFurnishing(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-700 w-full shadow-sm focus:ring-2 focus:ring-amber-500 focus:outline-none">
                            <option value="">Property Status</option>
                            <option value="ready">Ready to Move</option>
                            <option value="under-construction">Under Construction</option>
                            <option value="new-launch">New Launch</option>
                          </select>
                          <select aria-label="Investment Type" value={rera} onChange={(e)=>setRera(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-700 w-full shadow-sm focus:ring-2 focus:ring-amber-500 focus:outline-none">
